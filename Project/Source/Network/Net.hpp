...
namespace Algorithm {
	...
	template<class T>
	T Lerp(T A, T B, float per) { return A + (B - A) * per; }
	...
	public:
		InputControl operator+(const InputControl& o) const noexcept {
			InputControl Answer;
			Answer.AimVec = this->AimVec + o.AimVec;
			return Answer;
		}
		InputControl operator-(const InputControl& o) const noexcept {
			InputControl Answer;
		...
		InputControl operator*(float per) const noexcept {
			InputControl Answer;
			Answer.AimVec = this->AimVec * per;
			return Answer;
		}
	};
	class MoveInfo {
	public:
		Algorithm::Vector3DX repos{};		// 反映用座標
		Algorithm::Vector3DX pos{};		// 反映用座標
		Algorithm::Vector3DX vec{};		// 加速
	public:
...
namespace NetWork {
	...
	private:
		...
		void				SendData(IPDATA& Ip, int SendPort, const T& Data) noexcept {
			this->m_UDPCore.SendDataUDP(Ip, SendPort, &Data, sizeof(T));
		}
	public:
		// 送信
		template<class T>
		void				SendData(const T& Data) noexcept { SendData(this->m_SendIP, this->m_SendPort, Data); }
		// 受信
		template<class T>
		...
		// 返送
		template<class T>
		void				ReturnData(const T& Data) noexcept { SendData(this->m_RecvIp, this->m_RecvPort, Data); }
	};
	...
	public:
		void			SetMyPlayer(const Algorithm::InputControl& input, const Algorithm::MoveInfo& move, int32_t freeData[10]) noexcept {
			this->m_Input = input;
			this->m_move = move;
	...
	public:
		void			Int(NetTime Tick) noexcept {
			...
			this->m_PrevTime = Direct2DLib::Instance()->currentTime();
		}
		void			UpdateLocalData(const PlayerSendData& Players, bool TimerStart) noexcept {
			LONGLONG NowTime = Direct2DLib::Instance()->currentTime();
	...
	//クライアント専用
	class ClientControl {
		LONGLONG				m_CannotConnectTimer{ 0 };
		int						m_Port{ 0 };
